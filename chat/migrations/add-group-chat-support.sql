...
drop policy if exists "Users can view room memberships where they are members" on chat_room_members;
drop policy if exists "Users can request to join rooms" on chat_room_members;
drop policy if exists "Admins can manage room members" on chat_room_members;
drop policy if exists "Admins can add members to rooms" on chat_room_members;

-- Allow users to view memberships in rooms they're part of
create policy "Users can view room memberships where they are members"
...
create policy "Admins can add members to rooms"
  on chat_room_members for insert
  with check (
    exists (
      select 1 from chat_room_members crm_admin
