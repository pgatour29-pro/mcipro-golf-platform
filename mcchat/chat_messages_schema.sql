...
-- Fixed, production-grade schema for MyCaddyPro chat (LINE-style)
...
create table if not exists public.messages (
  ...
  body text,                           -- message body (text/system); null for pure media
...
-- Initialize receipts for all participants on new message
...
begin
  ...
  select new.id, cp.user_id,
         case when cp.user_id = new.sender_id then new.created_at else null end
...
-- ============ TYPING & TOKENS (optional tables kept minimal here) ============
...
-- conversations: only participants can read; creator inserts; admins update
...
create policy conversations_update_admin on public.conversations
  for update using (
    exists (
      select 1 from public.conversation_participants cp
      where cp.conversation_id = id and cp.user_id = auth.uid() and cp.role = 'admin'
    )
  );
...
-- ============ RPC: ensure direct conversation ============
...
begin
  ...
  insert into public.conversations(is_group, created_by) values(false, a) returning id into convo;
