...
(function () {
  ...
  var G3 = {
    ...
    /* v1283 - rail labels are baked in at build() time, so switching language left the WHOLE
       rail in the old language until a reload (the phone drawer switched instantly - that is the
       mobile/desktop mismatch Pete sees). build() MOVES the cubes grid so it can never re-run:
       relabel in place instead. Called from updateLanguage(). */
    relabel: function () {
      var rail = document.getElementById('g3Rail'); if (!this._built || !rail) return;
      RAIL.forEach(function (it) {
        if (it.grp !== undefined) return;
        var el = rail.querySelector('.g3-it[data-tab="' + (it.tab || '') + '"][data-act="' + (it.act || '') + '"]');
        if (!el) return;
        var label = it.k ? T(it.k, it.fb) : it.fb;
        var sp = el.querySelector('span:not(.material-symbols-outlined)');
        if (sp) sp.textContent = label;
        el.title = label;
      });
      var grps = rail.querySelectorAll('.g3-grp'), gi = 0;
      RAIL.forEach(function (it) { if (it.grp) { var g = grps[gi++]; if (g) g.textContent = T(it.grp, it.fb); } });
      this.syncNav(this._tab);
    },
