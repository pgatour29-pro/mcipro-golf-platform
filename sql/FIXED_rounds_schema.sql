-- URGENT FIX: rounds table schema - CORRECTED VERSION
-- Fixed: golfer_id now uses TEXT (for LINE user IDs), not UUID
...
BEGIN
    ...
    -- Add golfer_id if it doesn't exist (TEXT for LINE user IDs)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'rounds' AND column_name = 'golfer_id') THEN
        ALTER TABLE rounds ADD COLUMN golfer_id TEXT;  -- FIXED: TEXT instead of UUID
        COMMENT ON COLUMN rounds.golfer_id IS 'LINE user ID (text)';
    END IF;
...
-- Copy user_id to golfer_id for compatibility (if user_id exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'rounds' AND column_name = 'user_id') THEN
        UPDATE rounds
        SET golfer_id = user_id
...
BEGIN
    ...
    RAISE NOTICE '✅ Added columns with CORRECT data types (golfer_id = TEXT)';
    RAISE NOTICE '✅ Created performance indexes';
    RAISE NOTICE '✅ Migrated existing data';
    RAISE NOTICE '✅ Compatible with LINE user IDs (text format)';
END $$;
