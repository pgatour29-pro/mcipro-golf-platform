-- FIX LEADERBOARD V9 - Points-based system with correct rounds count
-- 1st = 25pts, 2nd = 15pts, 3rd = 10pts, 4th = 7pts, 5th = 5pts
-- Rounds = count of distinct events (using event_id or date if NULL)
-- Excludes: Bubba Gump, Willy, total_net < 10 (test data)
-- Season starts: Nov 1, 2025
...
BEGIN
  ...
  WITH valid_scorecards AS (
    SELECT
      ...
      COALESCE(sc.event_id, 'solo_' || sc.player_id || '_' || DATE(sc.started_at)::TEXT) as round_key
    FROM scorecards sc
    WHERE sc.total_net IS NOT NULL
      AND sc.total_net >= 10
      AND sc.player_name NOT IN ('Bubba Gump', 'Willy')
  ...
  event_positions AS (
    SELECT
      ...
      vs.player_name,
      vs.total_net,
      ROW_NUMBER() OVER (PARTITION BY vs.round_key ORDER BY vs.total_net ASC) as position
  ...
  player_points AS (
    SELECT
      ...
      SUM(CASE
        ...
        ELSE 0
      END) as points,
      COUNT(DISTINCT ep.round_key) as rounds
...
-- ============================================
-- SEASON STANDINGS - Nov 1, 2025 to present
-- ============================================
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    up.home_club::TEXT,
    (SELECT COUNT(DISTINCT COALESCE(sc.event_id, 'solo_' || sc.player_id || '_' || DATE(sc.started_at)::TEXT))
     FROM scorecards sc WHERE sc.player_id = up.line_user_id)::BIGINT,
    ARRAY(
...
-- TEST
-- ============================================
SELECT 'V9: Points system with correct rounds count (distinct events)' as status;
