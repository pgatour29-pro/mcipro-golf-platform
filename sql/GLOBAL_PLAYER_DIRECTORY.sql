-- ============================================================================
-- GLOBAL PLAYER DIRECTORY SYSTEM
-- ============================================================================
-- Created: 2025-12-11
-- Purpose: Unified player search and directory across ALL societies
-- Features:
--   - Unified view combining user_profiles and profiles tables
--   - Full-text search with fuzzy matching
--   - Advanced filters (society, handicap, home course)
--   - Player statistics and connections
-- ============================================================================
...
SELECT
    ...
    COALESCE(up.name, p.display_name, p.username) as player_name,
    p.display_name,
    p.username,
    p.avatar_url,
    ...
    (
        SELECT COUNT(DISTINCT sm.society_name)
        FROM society_members sm
        WHERE sm.golfer_id = COALESCE(up.line_user_id, p.line_user_id)
        AND sm.status = 'active'
    ) as society_count,
    ...
    COALESCE(up.profile_data, p.profile_data, '{}'::jsonb) as profile_data,
...
WHERE COALESCE(up.name, p.display_name, p.username) IS NOT NULL
  AND COALESCE(up.name, p.display_name, p.username) != '';
...
BEGIN
    RETURN jsonb_build_object(
        ...
        'total_societies', (SELECT COUNT(DISTINCT society_name) FROM society_members WHERE status = 'active'),
        ...
        'avg_rounds_per_player', (SELECT AVG(total_rounds)::numeric(10,2) FROM global_players WHERE total_rounds > 0),
...
-- VERIFICATION
-- ============================================================================
SELECT 'GLOBAL_PLAYER_DIRECTORY.sql deployed successfully' as status;
