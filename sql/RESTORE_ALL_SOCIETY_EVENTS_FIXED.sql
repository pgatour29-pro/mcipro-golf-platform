...
-- =====================================================
-- Created: 2025-11-01 (FIXED for UUID organizer_id)
...
-- =====================================================
-- STEP 1: Check current schema and find organizer UUID
-- =====================================================
...
DECLARE
    ...
    event_count INTEGER;
BEGIN
    -- Try to find Pete's organizer UUID from existing events or user_profiles
    -- If society_events uses UUID, we need to find the correct one

    RAISE NOTICE '========================================';
    RAISE NOTICE 'Checking for existing organizer ID...';
    RAISE NOTICE '========================================';

    -- Check if there are any existing events to get the organizer_id format
    SELECT COUNT(*) INTO event_count FROM society_events;
    RAISE NOTICE 'Current event count: %', event_count;

END $$;

-- =====================================================
-- STEP 2: Clean up any partial/duplicate data
-- =====================================================

-- Since organizer_id is UUID, we need to delete by date range only
DELETE FROM society_events
WHERE date >= '2025-10-20'
  AND date <= '2025-11-30';

-- =====================================================
-- STEP 3: RESTORE OCTOBER 2025 EVENTS
-- NOTE: Using NULL for organizer_id since we don't have the UUID
-- The organizer_name will still show "Travellers Rest Golf Group"
-- =====================================================
...
(
  ...
  0,
  80,
  NULL,  -- Will need to update with correct UUID
...
-- =====================================================
-- STEP 4: RESTORE NOVEMBER 2025 EVENTS
-- =====================================================
...
SELECT '=========================================' as message;
SELECT 'SUCCESS! All TRGG events restored.' as message;
SELECT '=========================================' as message;
