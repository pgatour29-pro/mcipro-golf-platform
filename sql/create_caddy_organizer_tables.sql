...
CREATE TABLE IF NOT EXISTS caddy_profiles (
    ...
    created_by TEXT, -- User who added caddy to system
...
-- caddy_profiles: Everyone can view, only admins can modify
...
CREATE POLICY "Users can add caddies"
ON caddy_profiles FOR INSERT
WITH CHECK (true);

CREATE POLICY "Users can update their own caddy entries"
ON caddy_profiles FOR UPDATE
USING (created_by = auth.uid()::text);

-- user_caddy_preferences: Users can only see/modify their own
ALTER TABLE user_caddy_preferences ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own caddy preferences"
ON user_caddy_preferences FOR SELECT
USING (user_id = auth.uid()::text);

CREATE POLICY "Users can insert own caddy preferences"
ON user_caddy_preferences FOR INSERT
WITH CHECK (user_id = auth.uid()::text);

CREATE POLICY "Users can update own caddy preferences"
ON user_caddy_preferences FOR UPDATE
USING (user_id = auth.uid()::text);

CREATE POLICY "Users can delete own caddy preferences"
ON user_caddy_preferences FOR DELETE
USING (user_id = auth.uid()::text);

-- caddy_bookings: Users can see/modify their own bookings
ALTER TABLE caddy_bookings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own caddy bookings"
ON caddy_bookings FOR SELECT
USING (user_id = auth.uid()::text);

CREATE POLICY "Users can insert own caddy bookings"
ON caddy_bookings FOR INSERT
WITH CHECK (user_id = auth.uid()::text);

CREATE POLICY "Users can update own caddy bookings"
