...
alter table public.golf_posts
  ...
  -- the WINDOW the recent slice is described in ("17 in the past 6 hours"). A window is a duration,
  -- never a stored instant: a pinned instant silently ages into "the past 20 hours" on its own.
  add column if not exists likes_boost_recent_hours integer not null default 6;
