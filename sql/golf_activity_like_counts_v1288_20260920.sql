...
AS $function$
  ...
  a as (
    ...
    union all
    select 'comment', c.author_id, p.id, null, c.body, c.created_at, p.author_id, 1
    ...
    select 'mention', m.author_id, p.id, null, coalesce(c.body, p.caption), m.created_at, p_user, 1
    ...
    union all
    select 'follow', f.follower_id, null, null, null, f.created_at, f.followee_id, 1 from public.golf_follows f
     where f.followee_id = any ((select ids from me)::text[]) and f.follower_id <> p_user
    union all
    select 'enquiry', e.buyer_id, null, e.listing_id, l.title, e.created_at, p_user, 1
    ...
    union all
    select 'offer', o.buyer_line_id, null, o.listing_id, l.title || case when o.offer_amount is not null then ' · ฿' || o.offer_amount else '' end, o.created_at, p_user, 1
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'post_id', a.post_id, 'listing_id', a.listing_id, 'n', a.n,
