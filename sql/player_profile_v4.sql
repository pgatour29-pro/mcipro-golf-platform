-- Player Profile V4 - Tightened filters
-- Only show rounds with gross >= 50 (real 18-hole rounds, not test/partial)
-- Stableford-only rounds: require total_stableford >= 18 OR hole_count >= 14
...
BEGIN
  SELECT json_build_object(
    ...
    'statistics', json_build_object(
      ...
      'avg_gross', (
        SELECT ROUND(AVG(sc.total_gross)::NUMERIC, 1) FROM scorecards sc
        WHERE sc.player_id = up.line_user_id
          AND sc.total_gross >= 50
          AND DATE(sc.started_at) >= '2025-12-01'
      ),
      'best_gross', (
        SELECT MIN(sc.total_gross) FROM scorecards sc
        WHERE sc.player_id = up.line_user_id
          AND sc.total_gross >= 50
          AND DATE(sc.started_at) >= '2025-12-01'
      ),
      'avg_net', (
        SELECT ROUND(AVG(sc.total_net)::NUMERIC, 1) FROM scorecards sc
        WHERE sc.player_id = up.line_user_id
          AND sc.total_net >= 40
          AND DATE(sc.started_at) >= '2025-12-01'
      ),
      'avg_stableford', (
        ...
        FROM (
          SELECT SUM(COALESCE(s.stableford_points, s.stableford, 0)) as s_total
          FROM scores s
          ...
          HAVING SUM(COALESCE(s.stableford_points, s.stableford, 0)) >= 18
        ) sub
      ),
      'best_stableford', (
        SELECT MAX(s_total)
      ...
      'last_round_date', (
        SELECT MAX(sc.started_at) FROM scorecards sc
        WHERE sc.player_id = up.line_user_id
          AND sc.total_gross >= 50
          AND DATE(sc.started_at) >= '2025-12-01'
      )
