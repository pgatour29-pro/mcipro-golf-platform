-- ============================================================================
-- v536 (2026-07-12): ANCHOR IS PERMANENT — remove the 20-round WHS takeover.
-- Pete's call ("go"): once a player has an anchor, the universal handicap works
-- off it FOREVER (incremental only). From-scratch WHS 8-of-20 now applies ONLY
-- to players with no handicap anywhere (no universal, no society hcp, no
-- profile hcp — true beginners bootstrapping from their first rounds).
-- Single change vs sql/universal_handicap_anchor_v535.sql:
--   `IF v_anchor IS NULL OR v_total_rounds >= 20 THEN` -> `IF v_anchor IS NULL THEN`
-- ============================================================================
