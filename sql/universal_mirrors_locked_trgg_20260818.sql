-- ============================================================================
-- 2026-08-18 (Pete): "for all TRGG members show their TRGG hcp as the primary
-- and not the universal." TRGG/masterscoreboard is the AUTHORITY for anyone who
-- has a locked TRGG row — the universal (and the user_profiles mirror every
-- fallback surface reads) now TRACKS that locked value instead of drifting
-- independently (Justin Carroll 10.3-vs-7.9 case + 9 more drifted players).
--
-- Piece 1: cascade trigger on society_handicaps — any INSERT/UPDATE of a locked
--          TRGG%/%MASTERSCORE% society row re-syncs the universal row + profile.
--          (Covers the masterscoreboard pull tool + manual imports.)
-- Piece 2: auto_update_society_handicaps_on_round universal branch — if a
--          locked TRGG/masterscore row exists, universal := that value; the
--          ANCHORED drift logic only applies to golfers with NO locked row.
--          (Covers round completions between imports.)
-- A MANUAL universal row still wins over everything (explicit pin, e.g. Pluto).
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Piece 1: cascade locked society hcp -> universal + profile
-- ---------------------------------------------------------------------------
...
DROP TRIGGER IF EXISTS trg_sync_universal_to_locked ON public.society_handicaps;
CREATE TRIGGER trg_sync_universal_to_locked
AFTER INSERT OR UPDATE OF handicap_index ON public.society_handicaps
FOR EACH ROW EXECUTE FUNCTION public.sync_universal_to_locked_society();

-- ---------------------------------------------------------------------------
-- Piece 2: round-completion engine — universal mirrors the locked value.
-- Base: sql/universal_handicap_anchor_v536_no_whs_takeover.sql; the ONLY
-- change is the v_locked_hcp lookup + three-way branch in the universal section.
-- ---------------------------------------------------------------------------
