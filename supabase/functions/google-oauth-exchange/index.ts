...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!CLIENT_ID || !CLIENT_SECRET) {
      return json({ error: "Server not configured", detail: "Missing GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET" }, 500, origin);
    }

    // Use provided redirectUri or default - must match what was sent to Google
    const ALLOWED_REDIRECTS = ["https://mycaddipro.com/", "https://www.mycaddipro.com/"];
    const REDIRECT_URI = ALLOWED_REDIRECTS.includes(redirectUri) ? redirectUri : "https://www.mycaddipro.com/";
    ...
    // Transform Google profile to standard format
    ...
    const profile = {
      ...
      displayName: googleProfile.name || googleProfile.email?.split("@")[0] || "Google User",
    ...
    // Creates a public.profiles row keyed to GOOGLE-<id> (matching user_profiles) + an auth user,
    // then mints a magic-link token_hash. The Custom Access Token Hook injects line_id + profile_id.
    ...
    try {
      ...
      const effectiveId = `GOOGLE-${profile.id}`;
      const authEmail = `${effectiveId.toLowerCase()}@google-users.mycaddipro.com`;
      ...
      console.warn("[google-oauth-exchange] session establishment failed (non-fatal):", String(sessErr));
    }
