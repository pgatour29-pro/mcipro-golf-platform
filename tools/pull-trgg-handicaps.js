 ...
 *
 * THE BOOKMARKLET IS A LOADER (since v780). It fetches THIS file live from
 *   https://mycaddipro.com/tools/pull-trgg-handicaps.js
 * so it can never run a stale copy again (a stale bookmarklet re-froze the
 * MANUAL-locked players on 2026-07-31 hours after the v778 fix shipped).
 * After ANY edit here run `node tools/build-bookmarklet.js` — it syntax-checks,
 * copies this file into public/tools/ (the deployed URL), and rebuilds the
 * install page. The dialogs show VERSION so Pete can always tell what ran.
 *
 * SAFETY
...
(async () => {
  ...
  const msg = `TRGG handicaps updated ✅  (puller ${VERSION})\n\n` +
