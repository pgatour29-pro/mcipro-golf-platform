-- MIGRATION: Fix existing chat_messages table schema
-- This handles the case where table exists with wrong column names

-- Step 1: Check what columns exist and add/rename as needed
DO $$
DECLARE
  has_sender boolean;
...
BEGIN
  ...
  RAISE NOTICE 'has_sender: %, has_author_id: %', has_sender, has_author_id;

  -- If we have author_id but not sender, rename it
  IF has_author_id AND NOT has_sender THEN
    RAISE NOTICE 'Renaming author_id to sender...';
    ALTER TABLE public.chat_messages RENAME COLUMN author_id TO sender;
  END IF;

  -- If we don't have sender and don't have author_id, add sender
  IF NOT has_sender AND NOT has_author_id THEN
    RAISE NOTICE 'Adding sender column...';
    ALTER TABLE public.chat_messages ADD COLUMN sender uuid NOT NULL DEFAULT auth.uid();
  END IF;
...
-- Step 3: Add foreign key if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'chat_messages_room_id_fkey'
      AND table_name = 'chat_messages'
  ) THEN
...
-- Step 5: Drop and recreate RLS policies with correct column name
...
-- Create NEW policies that use "sender" column
...
-- Step 6: Fix or create the RPC function
...
GRANT EXECUTE ON FUNCTION public.ensure_direct_conversation(uuid) TO anon;
...
BEGIN
  ...
  IF sender_exists THEN
    RAISE NOTICE '✅ Chat schema migration complete!';
    RAISE NOTICE '✅ Tables: rooms, chat_messages (with sender column), room_members';
    RAISE NOTICE '✅ RPC: ensure_direct_conversation';
    RAISE NOTICE '✅ RLS policies: Applied with correct column names';
  ELSE
    RAISE EXCEPTION 'Migration failed: sender column still missing';
  END IF;
END $$;
